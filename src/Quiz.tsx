...
function Quiz() {
    ...
    const [multipleChoiceAnswers, setMultipleChoiceAnswers] = useState<VocabularyWord[]>(
        []
    );
    ...
    const getMultipleChoiceAnswers = (randomWord: VocabularyWord) => {
        ...
        while (newMultipleChoiceAnswers.length < 3) {
            ...
            const randomWord = vocabularyList.words[randomIndex];
            if (newMultipleChoiceAnswers.filter((word) => word.word === randomWord.word).length === 0) {
